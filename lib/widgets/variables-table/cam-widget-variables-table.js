define([
  ...
  function noop() {
    console.info('noop....');
    console.info.apply(console, ['noop'].concat(Array.prototype.slice.apply(arguments)));
  }
    ...
    return {
      ...
      link: function ($scope) {
        ...
        function hasChanged(v) {
          console.info('skip changes check', v, !$scope.variables[v]._copy);
          if (!$scope.variables[v]._copy) { return false; }
          var yep;
          var now = _getVar(v);
          console.info('copy for check', !$scope.variables[v]._copy);
        ...
        [
          ...
          $scope[name] = (angular.isFunction($scope[name]) ? $scope[name] : noop);
        });

        function validate(info, i) {
          // console.info('validate info', info);
        ...
        $scope.saveVariable = function (v) {
          var result = $scope.saveVar($scope.variables[v]);
          console.info('result', result);
          delete $scope.variables[v]._copy;
