    ...
    return {
      ...
      link: function ($scope) {
        ...
        function hasChanged(v) {
          if (!$scope.variables || !$scope.variables[v]._copy) { return false; }
          var yep;
          var now = _getVar(v);
        ...
        $scope.deleteVariable = function (v) {
          ...
          $scope.saveVar(info, v).then(function () {
            $scope.variable.splice(v);
          }, function (err) {
            console.error(err);
          });
        };
        ...
        $scope.downloadLink = $scope.downloadLink || (function (info) {
